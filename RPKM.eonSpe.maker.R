...
txdb1 <- makeTxDbFromGFF("./eonSpe/maker.gff3.MAKER.rename.clean.gtf",format="gtf")
#--------------------------------------------------
...
count.files = list.files("./htseq/", pattern="^J.*ES.*.maker.count2")
...
write.table(rpkm.all, "./rpkm.eonSpe.Aaron20170412.ES.maker.xls", quote=F, sep="\t", row.names=F, col.names=TRUE)
