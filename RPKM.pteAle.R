...
count.files = list.files("./htseq/", pattern="*.htseq.count")

lib.size.total = read.table("./bowtie2.Aaron20171208.lib.size", header=F,sep=" ", stringsAsFactors=F)
...
write.table(rpkm.all, "./rpkm.Aaron20171208.all.xls", quote=F, sep="\t", row.names=F, col.names=TRUE)
