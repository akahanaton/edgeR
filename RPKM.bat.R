...
count.files = list.files("./bowtie2/", pattern="^Y.*P[aA].*.count2")

lib.size.total = read.table("./bowtie2.Aaron20170509.lib.size", header=F,sep=" ", stringsAsFactors=F)
...
write.table(rpkm.all, "./rpkm.Aaron20170509.all.xls", quote=F, sep="\t", row.names=F, col.names=TRUE)
