...
txdb1 <- makeTxDbFromGFF("./eonSpe/maker.merge.evm.combined.gtf",format="gtf")
txdb2 <- makeTxDbFromGFF("./eonSpe/eonSpe.merge3.gmap/eonSpe.merge3.combined.gtf.exon.fa.sam.mu.collapsed.gff",format="gtf")
#--------------------------------------------------
# txdb2 <- makeTxDbFromGFF("./eonSpe/maker.gff3.MAKER.rename.clean",format="gff3")
#--------------------------------------------------
...
count.files = list.files("./htseq/", pattern="^J.*ES.*.maker.merge3.count")
...
write.table(rpkm.all, "./rpkm.eonSpe.Aaron20170412.ES.maker.merge3.xls", quote=F, sep="\t", row.names=F, col.names=TRUE)
